...
namespace UnfinityGames.U2DEX
{
	public static class AdvancedSnapping_UI
	{
		...
		private static GUIStyle verifiedTextStyle;
		private static GUIStyle unverifiedTextStyle;
		...
		private static void DrawGridOptions(int Width)
		{
		...
		private static void DrawSnapOptions(int Width)
		{
		...
		private static void DisplayThirdPartySupport()
		{
		...
		private static void DisplayHelpAndUpdates(int Width)
		{
			...
			{
				UnfinityUpdater.ShowUpdater(u2dexInfo.UpdateFileURL, 
											u2dexInfo.FullName, 
											u2dexInfo.version);
			}
		...
		private static void DisplayPluginToggle()
		{
			//Store the current state, before the user has a chance to toggle anything.
		...
		private static bool IsSupportedClass(string Name)
		{
		...
		private static string GetClassStringFromIndex(int index)
		{
			switch (index)
			{

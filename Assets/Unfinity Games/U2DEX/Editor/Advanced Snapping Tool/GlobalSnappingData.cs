...
{
	/// <summary>
	/// A static class that houses all of U2DEX's data that also handles 
	/// saving and loading said data to and from EditorPrefs.
	/// </summary>
	public static class GlobalSnappingData
	{
		private static SerializableGlobalSnappingData PrivateSnapData;
		...
		/// <summary>
		/// The scale of the grid that is displayed.  Note, we store this as an int, as we can't use enums with
		/// EditorPrefs easily.
		/// </summary>
		public static int GridScale
		{
			get
			{
				...
				if (PrivateSnapData != null)
				{
					return PrivateSnapData.gridScale;
				}
				else
				{
					return (int)u2dexGrid.GridScale.NearlyUnlimited;
				}
			}
			set
			{
				...
				if (PrivateSnapData != null)
				{
					PrivateSnapData.gridScale = value;
				}
			}
		}
